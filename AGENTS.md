...
- `docs/DESPLIEGUE_PRODUCCION.md` — el servidor real (`nexusback.ciaiuts.com`, Nodo 1, systemd + túnel de Cloudflare), cómo se despliega con `deploy/actualizar.sh` y qué lleva su `.env` (plantilla en `deploy/.env.produccion.example`).
